...
const toggleActive = (questionId) => {
    router.post(route('groups.questions.toggleActive', [props.group.id, questionId]), {}, {
        preserveScroll: true,
    });
};

const duplicateQuestion = (questionId) => {
    router.post(route('groups.questions.duplicate', [props.group.id, questionId]), {}, {
        preserveScroll: true,
        onSuccess: () => {
            router.visit(route('groups.questions.index', props.group.id));
        }
    });
};
...
const deleteQuestion = (questionId) => {
    if (confirm('Are you sure you want to delete this question? This action cannot be undone.')) {
        router.delete(route('groups.questions.destroy', [props.group.id, questionId]), {
            preserveScroll: true,
            onSuccess: () => {
                router.visit(route('groups.questions.index', props.group.id));
            }
...
const saveOrder = () => {
    ...
    form.post(route('groups.questions.reorder', props.group.id), {
        preserveScroll: true,
        onSuccess: () => {
            isReordering.value = false;
