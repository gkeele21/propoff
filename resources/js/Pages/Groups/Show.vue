    ...
    <AuthenticatedLayout>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div v-if="isCaptain" class="bg-surface overflow-hidden shadow-sm sm:rounded-lg border border-border">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-body mb-4">Quick Actions</h3>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            >
                                ...
                                <span class="font-semibold text-body">Invite Members</span>
                                <span class="text-sm text-muted text-center mt-1">Get invitation link & QR code</span>
                            </Link>

                            <Link
                                :href="route('groups.questions.index', group.id)"
                                class="flex flex-col items-center p-6 border-2 border-border rounded-lg hover:border-primary hover:bg-primary/10 transition"
                            >
                                ...
                                <span class="font-semibold text-body">Manage Questions</span>
                                <span class="text-sm text-muted text-center mt-1">Add, edit, or remove questions</span>
                            </Link>

                            <Link
                                :href="route('groups.grading.index', group.id)"
                                class="flex flex-col items-center p-6 border-2 border-border rounded-lg hover:border-success hover:bg-success/10 transition"
                            >
                                ...
                                <span class="font-semibold text-body">Set Answers</span>
                                <span class="text-sm text-muted text-center mt-1">Set answers and calculate scores</span>
                            </Link>

                            <Link
                                :href="route('groups.members.index', group.id)"
                                class="flex flex-col items-center p-6 border-2 border-border rounded-lg hover:border-muted hover:bg-surface-elevated transition"
                            >
                                ...
                                <span class="font-semibold text-body">Manage Members</span>
                                <span class="text-sm text-muted text-center mt-1">View and manage group members</span>
                            </Link>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="isCaptain && stats" class="bg-surface overflow-hidden shadow-sm sm:rounded-lg border border-border">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-body mb-4">Statistics</h3>

                        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                            <div class="text-center p-4 bg-warning/10 rounded-lg">
                                <p class="text-3xl font-bold text-warning">{{ stats.total_members }}</p>
                                <p class="text-sm text-muted">Members</p>
                            </div>
                            <div class="text-center p-4 bg-warning/20 rounded-lg">
                                <p class="text-3xl font-bold text-warning">{{ stats.total_captains }}</p>
                                <p class="text-sm text-muted">Captains</p>
                            </div>
                ...
                <div class="bg-surface overflow-hidden shadow-sm sm:rounded-lg border border-border">
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <h3 class="text-lg font-semibold text-body mb-4">About</h3>
                                <dl class="space-y-3">
                                    <div v-if="group.event">
                                        <dt class="text-sm font-medium text-muted">Event</dt>
                                        <dd class="mt-1 text-sm text-body">{{ group.event.name }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-sm font-medium text-muted">Description</dt>
                                        <dd class="mt-1 text-sm text-body">{{ group.description || 'No description' }}</dd>
                                    </div>
                                    <div v-if="group.creator">
                                        <dt class="text-sm font-medium text-muted">Created by</dt>
                                        <dd class="mt-1 text-sm text-body">{{ group.creator.name }}</dd>
                                    </div>
                                    <div v-if="group.is_public !== undefined">
                                        <dt class="text-sm font-medium text-muted">Visibility</dt>
                                        <dd class="mt-1">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                                :class="{
                                                    'bg-success/20 text-success': group.is_public,
                                                    'bg-surface-elevated text-body': !group.is_public,
                                                }"
                                            >
                                    ...
                                    <div v-if="isCaptain && group.grading_source">
                                        <dt class="text-sm font-medium text-muted">Grading Source</dt>
                                        <dd class="mt-1 text-sm text-body capitalize">{{ group.grading_source }}</dd>
                                    </div>
                                    <div v-if="isCaptain && group.entry_cutoff">
                                        <dt class="text-sm font-medium text-muted">Entry Cutoff</dt>
                                        <dd class="mt-1 text-sm text-body">
                                            {{ formatDateTime(group.entry_cutoff) }}
                                        </dd>
                                    </div>
                                    <div v-if="group.entries_count !== undefined">
                                        <dt class="text-sm font-medium text-muted">Total Entries</dt>
                                        <dd class="mt-1 text-sm text-body">{{ group.entries_count || 0 }}</dd>
                                    </div>
                                </dl>
                            </div>

                            <div>
                                <h3 class="text-lg font-semibold text-body mb-4">Group Code</h3>
                                <div class="bg-surface-elevated rounded-lg p-4">
                                    <div class="flex items-center justify-between">
                                        <div>
                                            <p class="text-sm text-muted mb-1">Share this code with others:</p>
                                            <p class="text-2xl font-mono font-bold text-body">{{ group.code || group.join_code }}</p>
                                        </div>
                                        <Button variant="primary" size="sm" @click="copyGroupCode">
                                            Copy
                                        </Button>
                                    </div>
                                </div>
                ...
                <div class="bg-surface overflow-hidden shadow-sm sm:rounded-lg border border-border">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-body mb-4">Members ({{ membersList.length }})</h3>
                                ...
                                <tbody class="bg-surface divide-y divide-border">
                                    <tr v-for="member in membersList" :key="member.id">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="font-medium text-body">{{ member.name }}</div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <span
                                                v-else
                                                class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-surface-elevated text-body"
                                            >
                                                Guest
                                            </span>
                                        </td>
                            ...
                            >
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="font-medium text-body truncate">
                                        {{ user.name }}
                                        <span v-if="user.id === group.created_by" class="text-xs text-muted">(Owner)</span>
                                    </div>
                ...
                <div v-if="!isCaptain && recentEntries" class="bg-surface overflow-hidden shadow-sm sm:rounded-lg border border-border">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-body mb-4">Recent Entries</h3>

                        <div v-if="recentEntries.length === 0" class="text-center py-8 text-muted">
                        ...
                        <div v-else class="space-y-3">
                            <div
                                ...
                                :key="entry.id"
                                class="flex items-center justify-between p-4 border border-border rounded-lg hover:shadow-md hover:bg-surface-elevated transition"
                            >
                                <div>
                ...
                <div v-if="isCaptain" class="bg-surface overflow-hidden shadow-sm sm:rounded-lg border-2 border-danger/30">
                    <div class="p-6">
