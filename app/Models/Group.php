...
class Group extends Model
{
    ...
    public function addCaptain($user)
    {
        ...
        // Reload the relationship so tests can see the new data
        $this->load('users');
    }
    ...
    /**
     * Check if entries are still allowed for this group.
     * Checks group-level cutoff first, then falls back to event-level cutoff.
     */
    public function acceptingEntries(): bool
    {
        // Check group-level cutoff first
        if ($this->entry_cutoff) {
            return now()->lessThanOrEqualTo($this->entry_cutoff);
        }

        // Fall back to event-level cutoff
        if ($this->event && $this->event->lock_date) {
            return now()->lessThanOrEqualTo($this->event->lock_date);
        }
