<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <PageHeader
                title="Edit Question"
                subtitle="Modify question for your group"
                :crumbs="[
                    { label: 'Dashboard', href: route('dashboard') },
                    { label: group.name, href: route('groups.show', group.id) },
                    { label: 'Questions', href: route('groups.questions.index', group.id) },
                    { label: 'Edit Question' }
                ]"
            >
                <template #metadata>
...
// Normalize options - handle both old format (strings) and new format (objects)
const normalizeOptions = (options) => {
    ...
    // If options are already objects with label/points, return as-is
    ...
    return options.map(opt => ({
        ...
        points: typeof opt === 'object' && opt.points ? opt.points : 0
    }));
};
...
const submit = () => {
    ...
    .patch(route('groups.questions.update', [props.group.id, props.question.id]), {
        onSuccess: () => {
